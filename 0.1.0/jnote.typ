#let (main, appendix, thebibliography) = {
  let gothic = (
    "Source Han Sans JP", "MS PGothic", "Hiragino Kaku Gothic Pro", "IPAexGothic", "Noto Sans CJK JP",
  )
  let mincho = (
    "Source Han Serif JP", "MS PMincho", "Hiragino Mincho Pro", "IPAexMincho", "Noto Serif CJK JP",
  )
  let english = ("Times New Roman", "New Computer Modern")

  let main(title: [タイトル], authors: [著者], abstract: none, date: none, body) = {
    // Set document metadata.
    set document(title: title)

    // Configure the page.
    set page(paper: "a4", margin: (top: 20mm, bottom: 27mm, x: 20mm))
    ...
    show ref: it => {
      if it.element != none and it.element.func() == math.equation {
        ...
        link(
          it.element.location(), numbering(
            it.element.numbering, ..counter(math.equation).at(it.element.location()),
          ),
        )
      } else {
  ...
  let thebibliography(bibobj, full: false, body) = {
    // Display bibliography.
    show bibliography: set text(9pt)
    show regex("[0-9a-zA-Z]"): set text(font: english)
    set bibliography(
      title: align(center, text(11pt, font: gothic)[参 考 文 献]), style: "jnote.csl", full: full,
    )
    bibobj
    body
  }
